...
const Lowest = ({ gamesLoaded, ratingOrderingLowest }) => {
  const ratingLowest = useSelector((state) => state.ratingLowest);
  useEffect(() => {
    ratingOrderingLowest();
  }, [gamesLoaded, ratingOrderingLowest]);

  return (
    <div className='container'>
      {ratingLowest &&
        ratingLowest.map((game) =>
          game.image ? (
